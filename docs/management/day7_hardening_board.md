...
| D7-07 Manual GitHub cloud proof workflow | DONE_WORKFLOW_ON_MAIN / BLOCKED_CLOUD_SECRET | `.github/workflows/day7-cloud-schema-proof.yml` is merged to `main` and can run `apply-and-verify` or `verify-only` after `PAWMATE_DB_MIGRATION_URL` is configured |
